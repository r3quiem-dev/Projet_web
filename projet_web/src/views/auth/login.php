...
<head>
    ...
    <style>
        ...
        /* --- CSS UIVERSE (Avec correctifs de placement) --- */
        .wrapper {
            --input-focus: #2d8cf0;
            --font-color: #323232;
            --font-color-sub: #666;
        ...
        .switch {
            ...
            align-items: center;
            /* Plus de height fixe pour éviter le chevauchement */
        }
        ...
        .slider:before {
            ...
            border: 2px solid var(--main-color); border-radius: 5px; left: 0px; bottom: 0px;
        ...
        /* TEXTES LOG IN / SIGN UP */
        .card-side::before {
            position: absolute; content: 'Log in'; top: 0; left: 25px; width: 60px;
            text-decoration: underline; color: var(--font-color); font-weight: 600; text-align: right;
        }
        .card-side::after {
            position: absolute; content: 'Sign up'; top: 0; right: 25px; width: 60px;
        ...
        .flip-card__inner {
            ...
            position: relative; background-color: transparent; perspective: 1000px;
            text-align: center; transition: transform 0.8s; transform-style: preserve-3d;
            margin-top: 40px; /* Espace pour ne pas toucher le bouton */
        }
        .toggle:checked ~ .flip-card__inner { transform: rotateY(180deg); }
        
        .flip-card__front, .flip-card__back {
            padding: 20px; position: absolute; display: flex; flex-direction: column;
            justify-content: center; -webkit-backface-visibility: hidden; backface-visibility: hidden;
            background: lightgrey; gap: 15px; border-radius: 5px; border: 2px solid var(--main-color);
            box-shadow: 4px 4px var(--main-color); width: 100%; height: 100%; box-sizing: border-box;
        }
        ...
        .flip-card__input {
            width: 100%; height: 40px; border-radius: 5px; border: 2px solid var(--main-color);
            background-color: var(--bg-color); box-shadow: 4px 4px var(--main-color);
            font-size: 15px; font-weight: 600; color: var(--font-color); padding: 5px 10px;
            outline: none; box-sizing: border-box;
        }
        .flip-card__input:focus { border: 2px solid var(--input-focus); }
        .flip-card__btn {
            width: 120px; height: 40px; border-radius: 5px; border: 2px solid var(--main-color);
            background-color: var(--bg-color); box-shadow: 4px 4px var(--main-color);
            font-size: 17px; font-weight: 600; color: var(--font-color); cursor: pointer;
        }
